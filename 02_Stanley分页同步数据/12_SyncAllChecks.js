...
if(statusNotes != null && statusNotes.size() > 0)
{
	...
	if(noteContent != null && noteContent != "" && noteModule == "Check")
	{
		// 使用toMap()解析状态数据
...
info "开始自动同步Checks，当前进度: " + currentOffset + " 条";
// 调用批量同步函数（带返回值）
syncResult = standalone.SyncCheckWithResult(currentOffset);
if(syncResult != null)
{
	finalProcessed = syncResult.get("finalProcessed");
	hasMore = syncResult.get("hasMore");
	errorIds = syncResult.get("errorIds");
	// 更新CheckSyncStatus记录（无论是否有错误都更新）
	statusData = Map();
	...
	statusNoteParams.put("Modules","Check");
	statusDataList = List();
	...
	if(hasMore == true)
	{
		...
		info "请在Zoho CRM Workflow中设置自动调度，调用 schedule.SyncAllChecks()，间隔5分钟";
	}
	else
	{
		info "所有数据已处理完成！";
