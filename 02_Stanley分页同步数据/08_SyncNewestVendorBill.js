void schedule.SyncNewestVendorBill()
{
...
statusNotes = zoho.crm.searchRecords("vendorLogs","(Name:equals:VendorBillSyncStatus)");
...
info "开始同步最新VendorBills，fromDate = " + fromDate;
try
{
	allItems = List();
	offset = 0;
	...
	for each  pageNum in pageList
	{
		...
		if(resp == null)
		{
			info "获取VendorBill列表失败，offset = " + offset;
			break;
		}
	...
	if(allItems == null || allItems.size() == 0)
	{
		...
		info "VendorBillSyncStatus 已更新，lastSyncDate = " + todayStr;
		return;
	}
	info "共获取 " + allItems.size() + " 条VendorBill，开始逐条同步";
	for each  item in allItems
	{
		billId = item.get("id");
		try
		{
			syncResult = standalone.SyncSingleVendorBill(billId);
			isSuccess = false;
			errorMsg = "未知错误";
			if(syncResult != null)
			{
			...
			else
			{
				...
				totalProcessed = totalProcessed + 1;
				info "VendorBill同步失败: " + billId + " - " + errorMsg;
			}
		}
		catch (e)
		{
