void schedule.SyncNewestVendorPayment()
{
...
statusNotes = zoho.crm.searchRecords("vendorLogs","(Name:equals:VendorPaymentSyncStatus)");
...
info "开始同步最新VendorPayments，fromDate = " + fromDate;
try
{
	allItems = List();
	offset = 0;
	...
	if(allItems == null || allItems.size() == 0)
	{
		...
		info "VendorPaymentSyncStatus 已更新，lastSyncDate = " + todayStr;
		return;
	}
	info "共获取 " + allItems.size() + " 条VendorPayment，开始逐条同步";
	...
	{
		info "失败VendorPayment ID（共 " + errorIds.size() + " 条）: " + errorIds;
	}
	statusData = Map();
	statusData.put("lastSyncDate",todayStr);
	statusData.put("lastSyncTime",zoho.currenttime.toString());
