...
if(statusNotes != null && statusNotes.size() > 0)
{
	...
	if(noteContent != null && noteContent != "" && noteModule == "Purchase_Orders")
	{
		// 使用toMap()解析状态数据
...
info "开始自动同步Purchase Orders，当前进度: " + currentOffset + " 条";
// 调用批量同步函数（带返回值）
...
if(syncResult != null)
{
	...
	if(hasMore == true)
	{
		...
		info "请在Zoho CRM Workflow中设置自动调度，调用 schedule.SyncAllPOs()，间隔5分钟";
	}
	else
	{
		info "所有数据已处理完成！";
