...
info "开始自动同步发票，当前进度: " + currentOffset + " 条";
// 调用批量同步函数（带返回值）
syncResult = standalone.SyncInvoiceWithResult(currentOffset);
if(syncResult != null)
{
	finalProcessed = syncResult.get("finalProcessed");
	hasMore = syncResult.get("hasMore");
	error400Ids = syncResult.get("error400Ids");
	// 更新InvoiceSyncStatus记录（无论是否有错误都更新）
	statusData = Map();
	...
	// 如果还有更多数据，提示设置自动调度
	if(hasMore == true && finalProcessed < 10000)
	{
		info "还有更多数据待处理，新的起始位置: " + finalProcessed;
		info "请在Zoho CRM Workflow中设置自动调度，调用 schedule.syncAllInvoices()，间隔5分钟";
	}
	else
	{
		info "所有数据已处理完成！";
