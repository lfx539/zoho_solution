void schedule.SyncAllVendorPayments()
{
// 从CRM获取当前进度
currentOffset = 0;
statusNoteId = null;
...
if(statusNotes != null && statusNotes.size() > 0)
{
	...
	if(noteContent != null && noteContent != "" && noteModule == "VendorPayment")
	{
		// 使用toMap()解析状态数据
...
info "开始自动同步VendorPayments，当前进度: " + currentOffset + " 条";
// 调用批量同步函数（带返回值）
syncResult = standalone.SyncVendorPaymentWithResult(currentOffset);
if(syncResult != null)
{
	finalProcessed = syncResult.get("finalProcessed");
	hasMore = syncResult.get("hasMore");
	errorIds = syncResult.get("errorIds");
	// 更新VendorPaymentSyncStatus记录（无论是否有错误都更新）
	statusData = Map();
	...
	statusNoteParams.put("Modules","VendorPayment");
	statusDataList = List();
	...
	if(hasMore == true)
	{
		...
		info "请在Zoho CRM Workflow中设置自动调度，调用 schedule.SyncAllVendorPayments()，间隔5分钟";
	}
	else
	{
		info "所有数据已处理完成！";
