void schedule.SyncNewestInvoice()
{
...
todayStr = "01/01/2026";
startOffset = 0;
// 从 otherLogs 读取 InvoiceSyncStatus（仅 Modules = Invoice 的记录）
statusNotes = zoho.crm.searchRecords("otherLogs","(Name:equals:InvoiceSyncStatus)");
if(statusNotes != null && statusNotes.size() > 0)
{
	...
	if(noteContent != null && noteContent != "" && noteModule == "Invoice")
	{
		statusMap = noteContent.toMap();
		if(statusMap != null)
		{
			// 续跑用的 nextOffset（没有则默认 0）
			...
			// 增量起始日期 lastSyncDate / lastSyncTime
...
// Zoho 当前日期 DD-Mon-YYYY 转为 dd/MM/yyyy
...
info "开始同步最新发票，fromDate = " + fromDate + "，本页 offset = " + startOffset;
try 
{
	// 只处理一页，避免超时
	...
	resp = invokeurl
	[
		url :baseUrl + "?limit=" + limit + "&offset=" + startOffset + "&q=" + qStr
		type :GET
		headers:headers
	];
	if(resp == null)
	{
		info "获取发票列表失败，offset = " + startOffset;
		return;
	}
	...
	if(items == null || items.size() == 0)
	{
		...
		statusNoteParams.put("Name","InvoiceSyncStatus");
		statusNoteParams.put("Modules","Invoice");
		...
		if(statusNoteId != null && statusNoteId != "")
		{
			invokeurl
			[
				url :"https://www.zohoapis.com.au/crm/v8/otherLogs/" + statusNoteId
				type :PUT
		...
		info "InvoiceSyncStatus 已更新，lastSyncDate = " + todayStr;
		return;
	}
	info "本页 " + items.size() + " 条发票（offset=" + startOffset + "），hasMore=" + hasMore + "，开始逐条同步";
	...
	info "本页同步完成，本次处理: " + totalProcessed + " 条（成功: " + totalSuccess + "，失败: " + totalFailed + "）";
	if(error400Ids != null && error400Ids.size() > 0)
	{
		info "400错误发票ID（共 " + error400Ids.size() + " 条）";
		automation.saveErrorIdsToLogs1(error400Ids.toString(),error400Ids.size(),startOffset,startOffset + totalProcessed,totalSuccess,totalFailed,"Invoice");
	}
	// 更新 InvoiceSyncStatus（保存续跑 offset 与 lastSyncDate）
	statusData = Map();
	if(hasMore == true)
	{
