...
fromDate = "01/01/2026";
todayStr = "01/01/2026";
// 从 Logs 读 lastSyncDate 或 lastSyncTime
...
info "开始同步最新PO，fromDate = " + fromDate;
try
{
	// 分页拉取
	allItems = List();
	...
	{
		info "没有新增PO（createdDate >= " + fromDate + "）";
		statusData = Map();
		...
		info "POSyncStatus 已更新，lastSyncDate = " + todayStr;
		return;
	}
	info "共获取 " + allItems.size() + " 条PO，开始逐条同步";
	...
	{
		info "失败PO ID（共 " + errorIds.size() + " 条）: " + errorIds;
	}
	statusData = Map();
	statusData.put("lastSyncDate",todayStr);
	statusData.put("lastSyncTime",zoho.currenttime.toString());
