string standalone.SyncVendorBillWithResult(Int startOffset)
{
result = Map();
// 配置参数
baseUrl = "https://6508945.suitetalk.api.netsuite.com/services/rest/record/v1/vendorbill";
limit = 1000;
maxProcessCount = 220;
// 记录错误的Vendor Bill ID
errorIds = List();
...
hasMore = true;
offset = apiOffset;
info "开始批量同步Vendor Bill，起始位置: " + startOffset + "，API offset: " + apiOffset + "，跳过前 " + skipCount + " 条";
// 使用固定列表模拟循环（最多处理150页）
...
for each  pageNum in pageList
{
	...
	// 1. 获取Vendor Bill列表
	info "正在获取第 " + (offset / limit + 1) + " 页Vendor Bill列表（offset: " + offset + "）";
	...
	if(listResp == null)
	{
		info "获取Vendor Bill列表失败，offset: " + offset;
		break;
	}
	...
	info "获取到 " + items.size() + " 条Vendor Bill，开始处理";
	// 2. 逐条处理Vendor Bill
	itemIndex = 0;
	for each  item in items
	{
		...
		try
		{
			syncResult = standalone.SyncSingleVendorBill(vbId);
			// 解析返回结果
			isSuccess = false;
			errorMsg = "未知错误";
			if(syncResult != null)
			...
			else
			{
				...
				totalProcessed = totalProcessed + 1;
				info "Vendor Bill同步失败: " + vbId + " - " + errorMsg;
			}
		}
		catch (e)
		{
...
// 输出错误的Vendor Bill ID列表,保存错误ID到CRM vendorLogs模块
if(errorIds != null && errorIds.size() > 0)
{
	...
	try
	{
		...
		// 创建错误记录
		errorParams = Map();
		errorParams.put("Name","VendorBillSync_400Errors_" + startOffset + "_" + finalProcessed);
		errorParams.put("Modules","VendorBill");
	...
	info "错误Vendor Bill ID列表（共 " + errorIds.size() + " 条）: " + errorIds;
}
// 返回结果
