string standalone.SyncVendorPaymentWithResult(Int startOffset)
{
result = Map();
// 配置参数
baseUrl = "https://6508945.suitetalk.api.netsuite.com/services/rest/record/v1/vendorpayment";
limit = 1000;
maxProcessCount = 220;
// 记录错误的VendorPayment ID
errorIds = List();
// 统计变量
totalProcessed = 0;
totalSuccess = 0;
...
hasMore = true;
offset = apiOffset;
info "开始批量同步VendorPayment，起始位置: " + startOffset + "，API offset: " + apiOffset + "，跳过前 " + skipCount + " 条";
// 使用固定列表模拟循环（最多处理150页）
...
for each  pageNum in pageList
{
	...
	// 1. 获取VendorPayment列表
	info "正在获取第 " + (offset / limit + 1) + " 页VendorPayment列表（offset: " + offset + "）";
	...
	if(listResp == null)
	{
		info "获取VendorPayment列表失败，offset: " + offset;
		break;
	}
	...
	info "获取到 " + items.size() + " 条VendorPayment，开始处理";
	// 2. 逐条处理VendorPayment
	itemIndex = 0;
	for each  item in items
	{
		...
		try
		{
			syncResult = standalone.SyncSingleVendorPayment(paymentId);
			// 解析返回结果
			isSuccess = false;
			errorMsg = "未知错误";
			if(syncResult != null)
			...
			else
			{
				...
				if(errorStr != null)
				{
					errorIds.add(paymentId);
				}
				totalFailed = totalFailed + 1;
				totalProcessed = totalProcessed + 1;
				info "VendorPayment同步失败: " + paymentId + " - " + errorMsg;
			}
		}
		catch (e)
		{
			...
			totalFailed = totalFailed + 1;
			totalProcessed = totalProcessed + 1;
			info "VendorPayment处理异常: " + paymentId + " - " + e;
		}
		itemIndex = itemIndex + 1;
	}
...
// 输出错误的VendorPayment ID列表,保存错误ID到CRM vendorLogs模块
if(errorIds != null && errorIds.size() > 0)
{
	...
	try
	{
		...
		// 创建错误记录
		errorParams = Map();
		errorParams.put("Name","VendorPaymentSync_400Errors_" + startOffset + "_" + finalProcessed);
		errorParams.put("Modules","VendorPayment");
	...
	info "错误VendorPayment ID列表（共 " + errorIds.size() + " 条）: " + errorIds;
}
// 返回结果
