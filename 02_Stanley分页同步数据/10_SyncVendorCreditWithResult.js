string standalone.SyncVendorCreditWithResult(Int startOffset)
{
result = Map();
// 配置参数
...
hasMore = true;
offset = apiOffset;
info "开始批量同步VendorCredit，起始位置: " + startOffset + "，API offset: " + apiOffset + "，跳过前 " + skipCount + " 条";
// 使用固定列表模拟循环（最多处理150页）
...
for each  pageNum in pageList
{
	...
	// 1. 获取VendorCredit列表
	info "正在获取第 " + (offset / limit + 1) + " 页VendorCredit列表（offset: " + offset + "）";
	...
	info "获取到 " + items.size() + " 条VendorCredit，开始处理";
	// 2. 逐条处理VendorCredit
	itemIndex = 0;
	for each  item in items
	{
		...
		try
		{
			...
			else
			{
				...
				totalProcessed = totalProcessed + 1;
				info "VendorCredit同步失败: " + creditId + " - " + errorMsg;
			}
		}
		catch (e)
		{
			...
			totalFailed = totalFailed + 1;
			totalProcessed = totalProcessed + 1;
			info "VendorCredit处理异常: " + creditId + " - " + e;
		}
		itemIndex = itemIndex + 1;
	}
...
// 输出错误的VendorCredit ID列表,保存错误ID到CRM vendorLogs模块
if(errorIds != null && errorIds.size() > 0)
{
	...
	try
	{
		...
		// 创建错误记录
		errorParams = Map();
		errorParams.put("Name","VendorCreditSync_400Errors_" + startOffset + "_" + finalProcessed);
		errorParams.put("Modules","VendorCredit");
	...
	info "错误VendorCredit ID列表（共 " + errorIds.size() + " 条）: " + errorIds;
}
// 返回结果
