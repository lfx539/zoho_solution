string standalone.SyncCheckWithResult(Int startOffset)
{
result = Map();
// 配置参数
...
hasMore = true;
offset = apiOffset;
info "开始批量同步Check，起始位置: " + startOffset + "，API offset: " + apiOffset + "，跳过前 " + skipCount + " 条";
// 使用固定列表模拟循环（最多处理150页）
...
for each  pageNum in pageList
{
	...
	// 1. 获取Check列表
	info "正在获取第 " + (offset / limit + 1) + " 页Check列表（offset: " + offset + "）";
	...
	info "获取到 " + items.size() + " 条Check，开始处理";
	// 2. 逐条处理Check
	itemIndex = 0;
...
// 输出错误的Check ID列表,保存错误ID到CRM vendorLogs模块
if(errorIds != null && errorIds.size() > 0)
{
	...
	try
	{
		...
		// 创建错误记录
		errorParams = Map();
		errorParams.put("Name","CheckSync_400Errors_" + startOffset + "_" + finalProcessed);
		errorParams.put("Modules","Check");
	...
	info "错误Check ID列表（共 " + errorIds.size() + " 条）: " + errorIds;
}
// 返回结果
