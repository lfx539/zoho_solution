void schedule.AutoSyncPOsFull()
{
// 从CRM获取当前进度
currentOffset = 0;
statusNoteId = null;
statusNotes = zoho.crm.searchRecords("Logs","(Name:equals:POSyncStatus)");
...
info "开始自动同步PO，当前进度: " + currentOffset + " 条";
// 调用批量同步函数（带返回值）
...
if(syncResult != null)
{
	...
	// 如果还有更多数据，提示设置自动调度
	if(hasMore == true && finalProcessed < 5000)
	{
		info "还有更多数据待处理，新的起始位置: " + finalProcessed;
		info "请在Zoho CRM Workflow中设置自动调度，调用 schedule.AutoSyncPOsFull()，间隔5分钟";
	}
	else
	{
		info "所有数据已处理完成！";
