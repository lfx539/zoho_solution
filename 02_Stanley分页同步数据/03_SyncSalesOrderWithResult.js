string standalone.SyncSalesOrderWithResult(Int startOffset)
{
result = Map();
// 配置参数
...
hasMore = true;
offset = apiOffset;
info "开始批量同步销售订单，起始位置: " + startOffset + "，API offset: " + apiOffset + "，跳过前 " + skipCount + " 条";
// 使用固定列表模拟循环（最多处理150页）
...
for each  pageNum in pageList
{
	...
	// 1. 获取订单列表
	info "正在获取第 " + (offset / limit + 1) + " 页订单列表（offset: " + offset + "）";
...
// 输出错误的订单ID列表,保存错误ID到CRM Notes模块
if(error400Ids != null && error400Ids.size() > 0)
{
	error400IdsStr = error400Ids.toString();
	error400IdsCount = error400Ids.size();
	automation.saveSalesOrderErrorIdsToLogs(error400IdsStr,error400IdsCount,startOffset,finalProcessed,totalSuccess,totalFailed);
	info "错误订单ID列表（共 " + error400Ids.size() + " 条）: " + error400Ids;
}
// 返回结果
