void schedule.syncAllSalesOrders()
{
// 从CRM获取当前进度
currentOffset = 0;
statusNoteId = null;
...
info "开始自动同步销售订单，当前进度: " + currentOffset + " 条";
// 调用批量同步函数（带返回值）
syncResult = standalone.SyncSalesOrderWithResult(currentOffset);
if(syncResult != null)
{
	finalProcessed = syncResult.get("finalProcessed");
	hasMore = syncResult.get("hasMore");
	error400Ids = syncResult.get("error400Ids");
	// 更新SalesOrderSyncStatus记录（无论是否有错误都更新）
	statusData = Map();
	...
	if(hasMore == true && finalProcessed < 10000)
	{
		...
		info "请在Zoho CRM Workflow中设置自动调度，调用 automation.syncAllSalesOrders()，间隔5分钟";
	}
	else
	{
		info "所有数据已处理完成！";
