...
import { putdata } from '../util/util.js';
...
const App = (props) => {
    ...
    const handleConvert = async () => {
        ...
        try {
            ...
            // 2. 更新 Deal 子表 Product_Interested_deal 的 Converted_To_Sales 字段
            let updatedInterestSubform = productInterestList.map(item => ({
                id: item.id,
                Converted_To_Sales: selectedIds.has(item.id),
            }));

            // 3. 更新 Deal 子表 Product_Purchased_deal（关联到 Products_Purchased）
