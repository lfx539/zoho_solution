void schedule.noSOReminder()
{
info "========== 开始执行 28天无订单提醒 ==========";
// ========== 1. 计算日期 ==========
today = zoho.currentdate;
cutoffDate = today.addDay(-28);
// 28天前
threeMonthsAgo = today.addDay(-90);
// 3个月前
...
coqlQuery = "SELECT id, Account_Name, Email, Owner, Owner.first_name, Owner.last_name, Last_SO_Date, Created_Time FROM Accounts WHERE (Created_Time >= '" + threeMonthsAgoDateTime + "' AND Last_SO_Date < '" + cutoffDateStr + "') and Owner.first_name not in ('Fangxin','SEAN')";
...
// 排除特定 Owner: Liu Fangxin 和 Sean Ren
coqlQuery2 = "select id, Account_Name, Email, Owner, Owner.first_name, Owner.last_name, Last_SO_Date, Created_Time from Accounts where ((Created_Time >= '" + threeMonthsAgoDateTime + "' and Last_SO_Date is null) and Owner.first_name not in ('Fangxin','SEAN'))";
...
for each account in dataList
{
	...
	// ========== 3.1 发邮件给 Account Owner ==========
	if(ownerId != null && ownerId != "")
	{
		ownerEmailSubject = "No Order Reminder: " + accountName + " - Last order " + lastSODate;
		...
		if(ownerRecord != null)
		{
			ownerEmailAddress = ownerRecord.get("email");
		}
		...
		if(ownerEmailAddress != null && ownerEmailAddress != "")
		{
			...
			info "已发送邮件给 Owner: " + ownerEmailAddress;
		}
	}
